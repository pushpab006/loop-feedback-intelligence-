...
import { settleAggregateOverloads } from "@prisma/orm-framework/components/components";
...
*/
function generateAggregateTypes(options) {
	...
	if (ambiguity !== void 0) throw sqlEmitterError("CONTRACT.AGGREGATE_DESCRIPTOR_AMBIGUOUS", `Aggregate '${ambiguity.operation}' has no single result type over codec '${ambiguity.codecId}': traits ${ambiguity.traits.join(", ")} all claim it.`, {
	...
	return `{ ${[...settled.operations].sort(([left], [right]) => left.localeCompare(right)).map(([operation, entry]) => {
		const members = [`readonly byCodec: { ${[...entry.byCodecId].filter(([codecId]) => contributedIds.has(codecId)).sort(([left], [right]) => left.localeCompare(right)).map(([codecId, descriptor]) => {
		...
		if (withoutInput !== void 0) members.push(`readonly withoutInput: ${renderAggregateResult(withoutInput, void 0, contributedIds)}`);
		if (entry.anyInput !== void 0) members.push(`readonly anyInput: ${renderAggregateResult(entry.anyInput, void 0, contributedIds)}`);
...
function renderAggregateResult(descriptor, inputCodecId, contributedIds) {
	const output = descriptor.output.kind === "self" ? inputCodecId : descriptor.output.codecId;
	if (output === void 0) throw sqlEmitterError("CONTRACT.AGGREGATE_DESCRIPTOR_AMBIGUOUS", `Aggregate '${descriptor.operation}' declares its own input's codec as its result but answers calls that carry no input.`, {
	...
	if (!contributedIds.has(output)) throw sqlEmitterError("CONTRACT.AGGREGATE_OUTPUT_CODEC_MISSING", `Aggregate '${descriptor.operation}' names result codec '${output}', which the stack does not contribute.`, {
...
function generateDocumentScopedStorageTypesType(types) {
	...
	for (const [typeName, typeInstance] of Object.entries(types)) {
		...
		typeEntries.push(`readonly ${serializeObjectKey(typeName)}: { readonly kind: "codec-instance"; readonly codecId: ${codecId}; readonly nativeType: ${nativeType}; readonly typeParams: ${typeParamsStr} }`);
	}
...
function generateTableLiteralType(table) {
	...
	for (const [colName, col] of Object.entries(table.columns)) {
		...
		columns.push(`readonly ${serializeObjectKey(colName)}: { readonly nativeType: ${nativeType}; readonly codecId: ${codecId}; readonly nullable: ${nullable}${defaultSpec}${typeParamsSpec}${typeRefSpec} }`);
	}
...
function generateTablesMapType(tables) {
	const tableEntries = [];
	for (const [tableName, table] of Object.entries(tables).sort(([a], [b]) => a.localeCompare(b))) tableEntries.push(`readonly ${serializeObjectKey(tableName)}: ${generateTableLiteralType(table)}`);
...
//# sourceMappingURL=dist-ZK6oeI5e.mjs.map